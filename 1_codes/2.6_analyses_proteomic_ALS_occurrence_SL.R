...
# Test 1 : Just proteins + covariates -----
## Data preparation ----
...
rm(ranger_grid, ranger_learners)
...
rm(ksvm_grid, ksvm_learners, ksvm_learners_list, ksvm_learners_screened_list, ksvm_library_combined)
...
rm(earth_grid, earth_names, earth_library_screened, earth_library)
...
rm(data_prep, X_sl, Y_sl, match_ids, 
   SL.glmnet.opt, SL.glmnet.opt, SL.ksvm.opt, SL.ranger.opt, SL.xgb.opt)

# Test 2 : Proteins + covariates + time from baseline to diagnosis -----
listWrappers()     # Check available algorithms
...
rm(ksvm_grid, ksvm_learners, ksvm_learners_screened, ksvm_learners_list, ksvm_learners_screened_list, ksvm_library_combined)
...
### top 20 ----
# GLMNet Champion (Alpha 0.9) - AUC: 0.618
SL.glmnet.a0.9 <- function(..., alpha = 0.9) {
  SL.glmnet(..., alpha = alpha)
}

# SVM Champion (Linear/Vanilladot, C=10, Screened) - AUC: 0.581
SL.ksvm.linear.c10 <- function(..., kernel = "vanilladot", C = 10) {
  SL.ksvm(..., kernel = kernel, C = C)
}

# Ranger Champion (mtry 15, nodesize 1, extratrees, Screened) - AUC: 0.563
# SL.ranger.m15.s1.extra <- function(..., mtry = 15, min.node.size = 1, splitrule = "extratrees") {
#   SL.ranger(..., mtry = mtry, min.node.size = min.node.size, splitrule = splitrule, num.trees = 1000)
# }

SL.ranger.m15.s1.extra <- function(Y, X, newX, family, ...) {
  n_prot <- ncol(X)
  mtry_safe <- min(15, n_prot) 
...
# XGBoost Champion (max_depth 2, Screened) - AUC: 0.537
SL.xgb.d2 <- function(..., max_depth = 2, eta = 0.01) {
  SL.xgboost(..., max_depth = max_depth, eta = eta, nrounds = 500)
}

# Earth Champion (Degree 2, Screened) - AUC: 0.584
SL.earth.d2 <- function(..., degree = 2) {
  SL.earth(..., degree = degree)
}

# Version ultra-robuste de Polymars
SL.polymars.safe <- function(Y, X, newX, family, ...) {
  X_mat <- as.matrix(X)
  newX_mat <- as.matrix(newX)
  
  # Utilisation de polyclass (pour classification binomiale)
  fit <- try(polspline::polyclass(Y, X_mat, cv = 5), silent = TRUE)
  
  # CORRECTION : inherits() avec un "s"
  if(inherits(fit, "try-error")) {
    # En cas d'échec, on retourne la moyenne de Y pour ne pas bloquer le SL
    pred <- rep(mean(Y), nrow(newX))
  } else {
    # On récupère la probabilité de la classe 1 (Case)
    pred <- polspline::ppolyclass(newX_mat, fit)[, 2]
  }
  
  out <- list(pred = pred, fit = fit)
  class(out) <- "SL.polymars"
  return(out)
}

# 2. Construction de la liste finale list_lib_2_b_optimized

list_lib_2_b_optimized <- list(
  # Modèles qui gèrent bien la dimensionnalité seuls
  "SL.leekasso",              # Incontournable (screening interne)
  "SL.glmnet.a0.9",           # Ton meilleur modèle actuel
  
  # Modèles nécessitant impérativement le screening top 20 (p > n)
  c("SL.ksvm.linear.c10", "screen.top20"),
  c("SL.ranger.m15.s1.extra", "screen.top20"),
  c("SL.xgb.d2", "screen.top20"),
  c("SL.earth.d2", "screen.top20"),
  
  # Modèles fragiles (Ajustés avec screening pour éviter les NA)
  c("SL.bayesglm", "screen.top20"),
  c("SL.stepAIC", "screen.top20"),
  c("SL.gam", "screen.top20"),
  c("SL.polymars.safe", "screen.top20"),
...
sl_fit_CV_2_b_opt_20 <- CV.SuperLearner(
  ...
  X = X_sl,                                                                     # predictors (proteins + covariates)
  family = binomial(),                                                          # binary outcome (0/1)
  SL.library = list_lib_2_b_optimized,                                          # list of algorithms to try
  id = match_ids,                                                               # matching
  method = "method.AUC",                                                        # performance method (AUC), default would be Non-Negative Least Squares
  cvControl = list(V = 10),                                                     # external CV: 10 folds to evaluate the overall performance (generalizability)
  innerCvControl = list(list(V = 10)),                                          # internal CV: 10 folds to estimate the optimal weights for each algorithm
...
list_lib_2_b_optimized_10 <- list(
  ...
  # Modèles nécessitant impérativement le screening top 20 (p > n)
  c("SL.ksvm.linear.c10", "screen.top10"),
  c("SL.ranger.m15.s1.extra", "screen.top10"),
  c("SL.xgb.d2", "screen.top10"),
  c("SL.earth.d2", "screen.top10"),
  
  # Modèles fragiles (Ajustés avec screening pour éviter les NA)
  c("SL.bayesglm", "screen.top10"),
  c("SL.stepAIC", "screen.top10"),
  c("SL.gam", "screen.top10"),
  c("SL.polymars.safe", "screen.top10"),
...
sl_fit_CV_2_b_opt_10 <- CV.SuperLearner(
  ...
  X = X_sl,                                                                     # predictors (proteins + covariates)
  family = binomial(),                                                          # binary outcome (0/1)
  SL.library = list_lib_2_b_optimized_10,                                       # list of algorithms to try
  id = match_ids,                                                               # matching
  method = "method.AUC",                                                        # performance method (AUC), default would be Non-Negative Least Squares
  cvControl = list(V = 10),                                                     # external CV: 10 folds to evaluate the overall performance (generalizability)
  innerCvControl = list(list(V = 10)),                                          # internal CV: 10 folds to estimate the optimal weights for each algorithm
...
sl_fit_CV_2_c_opt_20 <- CV.SuperLearner(
  ...
  X = X_sl,                                                                     # predictors (proteins + covariates)
  family = binomial(),                                                          # binary outcome (0/1)
  SL.library = list_lib_2_c_optimized_reduit,                                   # list of algorithms to try
  id = match_ids,                                                               # matching
  method = "method.AUC",                                                        # performance method (AUC), default would be Non-Negative Least Squares
  cvControl = list(V = 10),                                                     # external CV: 10 folds to evaluate the overall performance (generalizability)
  innerCvControl = list(list(V = 10)),                                          # internal CV: 10 folds to estimate the optimal weights for each algorithm
...
rm(list_lib_2_c_optimized_reduit)
...
sl_fit_CV_2_c_opt_10 <- CV.SuperLearner(
  ...
  X = X_sl,                                                                     # predictors (proteins + covariates)
  family = binomial(),                                                          # binary outcome (0/1)
  SL.library = list_lib_2_c_optimized_reduit_10,                                # list of algorithms to try
  id = match_ids,                                                               # matching
  method = "method.AUC",                                                        # performance method (AUC), default would be Non-Negative Least Squares
  cvControl = list(V = 10),                                                     # external CV: 10 folds to evaluate the overall performance (generalizability)
  innerCvControl = list(list(V = 10)),                                          # internal CV: 10 folds to estimate the optimal weights for each algorithm
...
rm(X_sl, Y_sl, match_ids, screen.top10, screen.top20, 
   SL.glmnet.a0.9, SL.ksvm.linear.c10, SL.ranger.m15.s1.extra, SL.xgb.d2, SL.earth.d2, SL.polymars.safe, 
   list_lib_2_c_optimized_reduit_10)

# assemblage et export ----
results_proteomic_ALS_occurrence_SL <-
  list(
    test_1 = list(
      sl_fit_CV_1_a = sl_fit_CV_1_a, 
...
rm(sl_fit_CV_1_a, 
    ...
    sl_fit_CV_ksvm_2, 
    sl_fit_CV_earth_2, 
    sl_fit_CV_2_b_opt_20, 
    sl_fit_CV_2_b_opt_10, 
    sl_fit_CV_2_c_opt_20, 
    sl_fit_CV_2_c_opt_10)
