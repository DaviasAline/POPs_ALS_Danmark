...
The aim of this sensitivity analysis is to investigate if differences observed between the Danish and Finnish cohorts are due to differences in duration of follow-up and age at baseline 
We re-ran the models by selecting the Finnish cases with follow-up and age at baseline similar to the Danish cases 
...
results adjusted on the cohort (4 categories), adjusted for sex, diagnosis age, smoking status, bmi and marital_status
These resuts are not trustful because sample size of MFH is too small (n=11)
```{r}
#| echo: false
#| message: false
#| warning: false
...
Results of sensivitity analysis 1 may be driven by the Danish cases. 
Here, we ran adjusted and stratified analyses depending on 
effects of baseline age, follow-up duration just among the Finnish population
All models are adjusted for sex, diagnosis age, smoking status, bmi and marital_status
```{r, fig.width=10}
#| echo: false
#| message: false
#| warning: false
...
Here, we tried baseline age and follow-up duration as interaction terms (continuous and categorical) just among the Finnish cases 
All models are adjusted for sex, diagnosis age, smoking status, bmi and marital_status
...
## Sensitivity analyses 5 - LASSO POPs selection 
### Model forcing the covariates to be included 
```{r}
#| echo: false
#| message: false
#| warning: false
plot(results_POPs_ALS_survival$sensi5$cv_fit_covar_forced)
```
This plots the cross-validation curve (red dotted line) along with upper and lower standard deviation curves along the 𝜆sequence (error bars). Two special values along the 𝜆sequence are indicated by the vertical dotted lines. 
...
#| message: false
#| warning: false
results_POPs_ALS_survival$sensi5$cv_fit_covar_forced$lambda.min 
```
...
#| message: false
#| warning: false
coef(results_POPs_ALS_survival$sensi5$cv_fit_covar_forced, s = "lambda.min")     
```
...
#| message: false
#| warning: false
results_POPs_ALS_survival$sensi5$cv_fit_covar_forced$lambda.1se
```
...
#| message: false
#| warning: false
coef(results_POPs_ALS_survival$sensi5$cv_fit_covar_forced, s = "lambda.1se")
```

### Model not forcing the covariates to be included 
```{r}
#| echo: false
#| message: false
#| warning: false
plot(results_POPs_ALS_survival$sensi5$cv_fit)
```
This plots the cross-validation curve (red dotted line) along with upper and lower standard deviation curves along the 𝜆sequence (error bars). Two special values along the 𝜆sequence are indicated by the vertical dotted lines. 
...
#| message: false
#| warning: false
results_POPs_ALS_survival$sensi5$cv_fit$lambda.min 
```
...
#| message: false
#| warning: false
coef(results_POPs_ALS_survival$sensi5$cv_fit, s = "lambda.min")     
```
...
#| message: false
#| warning: false
results_POPs_ALS_survival$sensi5$cv_fit$lambda.1se
```
...
#| message: false
#| warning: false
coef(results_POPs_ALS_survival$sensi5$cv_fit, s = "lambda.1se")
```

## New co-pollutant models with LASSO selected POPs (to get 95%CI and p-values)
```{r}
#| echo: false
#| message: false
#| warning: false
