...
#| message: false
#| warning: false
results_POPs_ALS_survival$danish$qgcomp$POPs_ALS_qgcomp_table_danish  |>
  flextable() |>
  flextable::font(fontname = "Calibri", part = "all") |> 
  fontsize(size = 10, part = "all") |>
  padding(padding.top = 0, padding.bottom = 0, part = "all") |>
  set_table_properties(align = "left")
results_POPs_ALS_survival$danish$qgcomp$POPs_ALS_qgcomp_figure_danish
```
...
#| message: false
#| warning: false
results_POPs_ALS_survival$danish$qgcomp$POPs_ALS_qgcomp_positive_table_danish  |>
  flextable() |>
  flextable::font(fontname = "Calibri", part = "all") |> 
  fontsize(size = 10, part = "all") |>
  padding(padding.top = 0, padding.bottom = 0, part = "all") |>
  set_table_properties(align = "left")
results_POPs_ALS_survival$danish$qgcomp$POPs_ALS_qgcomp_positive_figure_danish
```
...
wrap_plots(list(results_POPs_ALS_survival$danish$plot_adjusted_cox_gam_danish$PCB_4,
...
NEFL is not significantly associated with ALS occurence when the follow-up duration is >16 years. 
We don't see many associations in the follow-up group T1: I suspect that it is because of high heterogeneity in follow-up duration among this sub-group (0-11 years).
--> then I trust more the sensi 2 results where we just remove the cases (and their controls) with follow-up <5 years. 
