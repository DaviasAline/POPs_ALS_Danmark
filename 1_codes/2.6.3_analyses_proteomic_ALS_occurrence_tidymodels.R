...
xgb_id_t1 <- summary_t1$wflow_id[5]
...
set.seed(1996)
results_t2 <- workflow_map(wf_set_t2, fn = "fit_resamples", 
                           resamples = folds_t2,
                           metrics = metric, 
                           control = control_resamples(save_pred = TRUE), 
                           verbose = TRUE)
...
xgb_id_t2 <- summary_t2$wflow_id[4]
...
glmnet_t1_result <- extract_glmnet_coefs(
  ...
  cat("✓ Successfully extracted glmnet coefficients\n")
  cat("  Lambda optimal:", round(glmnet_t1_result$lambda_opt, 6), "\n")
  cat("  Variables sélectionnées:", glmnet_t1_result$n_selected, "\n")
  cat("  Protéines sélectionnées:", nrow(glmnet_t1_result$coefs_protein), "\n\n")
  
# Table Top 20 predictors (by |coefficient|)
cat("Top 20 predictors (by |coefficient|):\n")
t_glmnet_t1 <- glmnet_t1_result$coefs_protein |> 
    slice_head(n = 20) |>
    as.data.frame() |>
    flextable() |> 
...
# Plot
p_glmnet_t1 <- glmnet_t1_result$coefs_protein |>
    slice_head(n = 25) |>
    ...
    labs(title = "Test 1 — glmnet coefficients (top 25 predictors)",
...
print(xgb_t1_shap$shap_summary |> slice_head(n = 20))

# SHAP beeswarm plot
print(sv_importance(xgb_t1_shap$shap, kind = "beeswarm", max_display = 20) +
        labs(title = "Test 1 XGBoost — SHAP beeswarm (top 20)"))

# SHAP bar plot
print(sv_importance(xgb_t1_shap$shap, kind = "bar", max_display = 20) +
...
for (prot in top6_t1) {
  cat("\n→ SHAP dependence:", prot, "\n")
  print(sv_dependence(xgb_t1_shap$shap, v = prot) +
...
cat("Top 20 predictors (by |coefficient|):\n")
t_glmnet_t2 <- glmnet_t2_result$coefs_protein |> 
    slice_head(n = 20) |>
    as.data.frame() |>
    flextable() |> 
    colformat_double(digits = 2) |>
...
p_glmnet_t2 <- 
  glmnet_t2_result$coefs_protein |>
    ...
    scale_fill_manual(values = c("↑ ALS risk" = "firebrick", "↓ ALS risk" = "steelblue")) +
    geom_vline(xintercept = 0, linetype = "dashed") +
    labs(title = "Test 2 — glmnet coefficients (top 50 predictors)",
         x = "Coefficient (standardized)", y = NULL) +
    theme_minimal() + 
  theme(legend.position = "bottom")
...
# Top 20 variables by SHAP importance
print(xgb_t2_shap$shap_summary |> slice_head(n = 20))

# SHAP beeswarm plot
print(sv_importance(xgb_t2_shap$shap, kind = "beeswarm", max_display = 20) +
        labs(title = "Test 2 XGBoost — SHAP beeswarm (top 20)"))

# SHAP bar plot
print(sv_importance(xgb_t2_shap$shap, kind = "bar", max_display = 20) +
...
for (prot in top6_t2) {
  cat("\n→ SHAP dependence:", prot, "\n")
  print(sv_dependence(xgb_t2_shap$shap, v = prot) +
...
for (prot in top6_t2) {
  print(sv_dependence(xgb_t2_shap$shap, v = prot, color_var = "follow_up_no_na_y") +
...
cat("\n--- TEST 2 : INTERACTIONS follow_up × PROTEINS ---\n")
...
p_fu_shap <- ggplot(shap_by_fu, aes(x = fu_group, y = mean_abs_shap, fill = direction)) +
...
overlap_t1 <- intersect(glmnet_t1_top, xgb_t1_top)
cat("  Common top 20 proteins:", length(overlap_t1), "\n")
if (length(overlap_t1) > 0) {
  cat("  Proteins:", paste(overlap_t1, collapse = ", "), "\n")
}

cat("\nTest 2 — glmnet vs XGBoost overlap:\n")
overlap_t2 <- intersect(glmnet_t2_top, xgb_t2_top)
cat("  Common top 20 proteins:", length(overlap_t2), "\n")
if (length(overlap_t2) > 0) {
  cat("  Proteins:", paste(overlap_t2, collapse = ", "), "\n")
}


## Additional investigation: tree_depth = 2 for Test 2 ----
cat("INVESTIGATION: XGBoost tree_depth = 2 vs Optimal (Test 2)\n")

train_xgb_with_depth <- function(data, other_params, tree_depth_val, test_label) {
  ...
  X_mat <- data_model |> select(-als_num) |> as.matrix()
  Y_num <- as.numeric(data$als == "case")
  
  folds <- group_vfold_cv(data, group = match, v = 10)
  
  xgb_folds <- lapply(seq_len(nrow(folds)), function(i) {
    test_match_ids <- testing(folds$splits[[i]])$match
    which(data$match %in% test_match_ids)
  })
  ...
  xgb_cv_es <- xgb.cv(
    params = list(
      objective        = "binary:logistic",
      eval_metric      = "auc",
      max_depth        = tree_depth_val,
      eta              = other_params$learn_rate,
      colsample_bytree = other_params$mtry / ncol(X_mat),
      min_child_weight = other_params$min_n,
      gamma            = other_params$loss_reduction,
      subsample        = 0.8,
      nthread          = parallel::detectCores() - 1),
    data                  = xgb.DMatrix(data = X_mat, label = Y_num),
    nrounds               = 5000,
    folds                 = xgb_folds,
  ...
  return(tibble(
    ...
    auc_mean       = best_row$test_auc_mean,
    auc_std        = best_row$test_auc_std))
}

# Test different tree depths
depths_to_test <- c(1, 2, 3, 4, 5)
depth_results <- map_df(depths_to_test, function(d) {
  cat("  Testing tree_depth =", d, "...\n")
  train_xgb_with_depth(data_t2, results_proteomic_ALS_occurrence_tidymodels$test_2$best_xgb_params_t2, d, "Test 2")
})

cat("\nXGBoost performance across tree_depth (Test 2):\n")
print(depth_results)

p_depth <- ggplot(depth_results, aes(x = factor(tree_depth), y = auc_mean)) +
  geom_col(fill = "steelblue", alpha = 0.7, width = 0.6) +
  geom_errorbar(aes(ymin = auc_mean - auc_std, ymax = auc_mean + auc_std), width = 0.2) +
  geom_point(size = 3, color = "darkblue") +
  geom_text(aes(label = round(auc_mean, 3), y = auc_mean + auc_std + 0.01),
            vjust = 0, size = 4, fontface = "bold") +
  geom_vline(xintercept = results_proteomic_ALS_occurrence_tidymodels$test_2$best_xgb_params_t2$tree_depth + 0.5,
             linetype = "dashed", color = "red", linewidth = 1, alpha = 0.7) +
  labs(title = "Test 2 — XGBoost AUC vs tree_depth (CV)",
       x = "Tree Depth", y = "AUC",
       subtitle = paste("Red dashed line = optimal depth from tuning (", 
                        results_proteomic_ALS_occurrence_tidymodels$test_2$best_xgb_params_t2$tree_depth, ")", 
                        sep = "")) +
  ylim(0, 1) +
  theme_minimal()
print(p_depth)
...
# Conclusion
optimal_depth <- results_proteomic_ALS_occurrence_tidymodels$test_2$best_xgb_params_t2$tree_depth
optimal_auc <- depth_results$auc_mean[depth_results$tree_depth == optimal_depth]
depth2_auc <- depth_results$auc_mean[depth_results$tree_depth == 2]
delta_auc <- depth2_auc - optimal_auc

cat("\n--- CONCLUSION ---\n")
cat("Optimal tree_depth found:", optimal_depth, "→ AUC =", round(optimal_auc, 3), "\n")
cat("tree_depth = 2         → AUC =", round(depth2_auc, 3), "\n")
cat("Delta AUC (depth2 - optimal):", round(delta_auc, 4), "\n")

if (abs(delta_auc) < 0.02) {
  cat("✓ tree_depth = 2 performs similarly to optimal\n")
  cat("  → Shallower trees may be sufficient & more interpretable\n")
} else if (delta_auc > 0) {
  cat("✓ tree_depth = 2 actually performs BETTER than optimal!\n")
  cat("  → Your intuition was correct: depth=2 better captures the patterns\n")
} else {
  cat("✗ tree_depth = 2 underperforms vs optimal\n")
  cat("  → The optimal depth correctly uses deeper trees\n")
}

# Si depth=2 est compétitif, entrainer et analyser
if (abs(delta_auc) < 0.05) {
  cat("\n→ Training XGBoost with tree_depth = 2 for detailed SHAP analysis...\n")
  
  xgb_t2_depth2 <- results_proteomic_ALS_occurrence_tidymodels$test_2$best_xgb_params_t2 |> mutate(tree_depth = 2)
  xgb_t2_depth2_shap <- train_xgb_and_shap(
    data_t2,
    best_params = xgb_t2_depth2,
    best_trees  = depth_results$best_nrounds[depth_results$tree_depth == 2],
    test_label  = "Test 2 XGBoost depth=2")
  
  cat("\nTop 20 variables (tree_depth = 2):\n")
  print(xgb_t2_depth2_shap$shap_summary |> slice_head(n = 20))
  
  print(sv_importance(xgb_t2_depth2_shap$shap, kind = "bar", max_display = 25) +
          labs(title = "Test 2 XGBoost tree_depth=2 — SHAP importance"))
  
  # Dependence plots depth=2
  top6_depth2 <- xgb_t2_depth2_shap$shap_summary |> slice_head(n = 6) |> pull(Feature)
  for (prot in top6_depth2) {
    print(sv_dependence(xgb_t2_depth2_shap$shap, v = prot, color_var = "follow_up_no_na_y") +
            scale_color_viridis_c(name = "Follow-up\n(years)") +
            labs(title = paste("Test 2 depth=2 —", prot, "× follow_up")) +
            theme_minimal())
  }
}
...
rm(step_select_auc_top, bake.step_select_auc_top, prep.step_select_auc_top, 
   ...
   data_t1, folds_t1,
   rec_full_t1, rec_top20_t1, rec_top10_t1, ncol_preds_t1, 
   data_t2, folds_t2,
   rec_full_t2, rec_top20_t2, rec_top10_t2, ncol_preds_t2)
